...
import { MapPinIcon, Loader2, TrainIcon } from "lucide-react";
...
function TrainSchedule() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/trains_search", {
        origin,
        destination,
      });
      setResults(res.data);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
      <div className="flex items-center mb-6 border-b pb-4">
        <TrainIcon className="h-8 w-8 text-cyan-600 mr-3" />
        <h1 className="text-2xl font-bold text-gray-800">Train Schedule</h1>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="pl-10 block w-full rounded-md border border-gray-300 shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
          ...
          {results.length > 0 ? (
            ...
            <div className="text-center py-10 text-gray-500">
              {loading
                ...
                : "No train schedule to display. Please enter origin and destination stations."}
            </div>
          )}
        </div>
      </div>
